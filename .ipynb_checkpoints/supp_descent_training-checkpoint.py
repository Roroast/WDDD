...
scales = list(np.logspace(-6, 2, num = 9, base = np.e))
lr_df = pd.DataFrame([0.367879], columns = ['lr'])
scale_df = pd.DataFrame(scales, columns = ['scale'])

supp_descent_DF = supp_descent_DF.merge(lr_df, how = "cross").merge(scale_df, how = "cross")

experiment(supp_descent_DF)
supp_descent_DF['loss'] = supp_descent_DF['loss_values'].str[-1]
supp_descent_DF.to_pickle("supp_descent_DF.pkl")